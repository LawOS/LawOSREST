...
public class HeadOfficeManagement {
	...
	public String viewRecomPerBranch(@FormParam("BranchID") String BranchID) {
		...
		try {
			stmt = conn.createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT DISTINCT Recommendation FROM `appointment` WHERE BranchID="
							+ BranchID);
			response = GeneralServices.parseJSON(rs);
			rs.close();
		} catch (SQLException e) {
	...
	public String viewLegalopPerBranch(@FormParam("BranchID") String BranchID) {
		...
		try {
			stmt = conn.createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT DISTINCT LegalOpinion FROM `appointment` WHERE BranchID="
							+ BranchID);
			response = GeneralServices.parseJSON(rs);
			rs.close();
		} catch (SQLException e) {
	...
	public String getClientComApp(@FormParam("ID") String ID) {
		...
		try {
			stmt = conn.createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT * FROM `appointment` WHERE (ClientID='"
							+ ID + "' AND Completed=1)");
			response = GeneralServices.parseJSON(rs);
			rs.close();
		} catch (SQLException e) {
